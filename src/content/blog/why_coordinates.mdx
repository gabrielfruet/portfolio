...
We talked about much of the theorical stuff, but let's see how it works in practice.

In the end, we want to optimize some metric, that in our case is IoU. 

So we expect that lower loss values lead to higher IoU values. Let's check that.
...
We can see that although both losses have negative correlation with IoU, CIoU
loss has a much better correlation with IoU than L1 loss.

For example, in **L1 loss**, when the IoU = 0.4, we can see that the loss can ranges from **15 to 30**. In contrast,
**CIoU loss** ranges from **0.4 to 0.65**.